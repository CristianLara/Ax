...
sidebar_label: High-Dimensional Bayesian Optimization with Sparse Axis-Aligned Subspaces (SAASBO)
---
...
<LinkButtons
  githubUrl="https://github.com/facebook/ax/blob/main/tutorials/saasbo/saasbo.ipynb"
  colabUrl="https://colab.research.google.com/github/facebook/ax/blob/main/tutorials/saasbo/saasbo.ipynb"
/>

# High-Dimensional Bayesian Optimization with SAASBO

This tutorial shows how to use the Sparse Axis-Aligned Subspace Bayesian Optimization
...
We find that SAASBO performs well on problems with hundreds of dimensions. As we rely on
...
\[1\] D. Eriksson, M. Jankowiak. High-Dimensional Bayesian Optimization with Sparse
...
We can also instantiate a SAAS model via `Models.BOTORCH_MODULAR` by specifying a
`SaasFullyBayesianSingleTaskGP` as the `botorch_model_class`. This also gives us the
option to change several Pyro-specific parameters such as `num_samples` and
`warmup_steps`.
...
We have tools for cross-validation in Ax, but plotly doesn't render on Github so we make
...
As SAASBO places strong priors on the inverse lengthscales, we only expect parameters 19
